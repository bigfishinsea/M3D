...
# dataset = pd.read_csv('dataset_ANN_full.csv')
dataset = pd.read_csv('dataset_ANN_error.csv')
...
# 层节点数目（包括输入层、输出层）
model = neuralNetwork5Layer(inputnums, 150, 150, 150, 150, 150, outputnums)
if use_gpu:
...
# 保存模型
torch.save(model.state_dict(), './neural_network_150_2000_24000_error.pth')
# torch.jit.save(model, './neural_network_singlecom_test.pt')
# model.save('./neural_network_90_200.pt')
