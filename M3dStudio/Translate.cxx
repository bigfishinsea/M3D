...
void Translate::ExportBREP(const Handle(TopTools_HSequenceOfShape)& shapes)
{
	QString fileName = selectFileName(0, false);
	if (fileName.isEmpty())
		return;

	exportBREP(fileName, shapes);
}
...
bool Translate::importModel(Handle(TopTools_HSequenceOfShape)& retShapes, const Handle(AIS_InteractiveContext)& ic)
{
	myInfo = QString();
	QString fileName = selectFileName(0, true);
	if (fileName.isEmpty())
		return false;

	if (!QFileInfo(fileName).exists())
	{
		myInfo = QObject::tr("文件不存在").arg(fileName);
		return false;
	}
	...
	///根据选择的文件扩展名，确定format
	int format;
	if (QFileInfo(fileName).suffix().toLower() == "brep" || QFileInfo(fileName).suffix().toLower() == "brp")
		format = FormatBREP;
	else if (QFileInfo(fileName).suffix().toLower() == "step" || QFileInfo(fileName).suffix().toLower() == "stp")
		format = FormatSTEP;
	//else if (QFileInfo(fileName).suffix().toLower() == "stl")
	//	format = FormatSTL;
	else if (QFileInfo(fileName).suffix().toLower() == "iges" || QFileInfo(fileName).suffix().toLower() == "igs")
		format = FormatIGES;
	...
	return true; //displayShSequence(ic, shapes); 先不显示
}
...
TranslateDlg* Translate::getDialog(const int format, const bool import)
{
	...
	//QString allFilter = QObject::tr("*.stp");

	QStringList filters;
	if (import)
		filters << "*.brep" << "*.step" << "*.iges" << "*.stl" << "*.*";
	else 
		filters << "*.brep" << "*.step" << "*.iges" << "*.stl" << "*.xml";

	myDlg->setNameFilters(filters);

	if (import)
	{
		myDlg->setWindowTitle(QObject::tr("导入"));
		((QFileDialog*)myDlg)->setFileMode(QFileDialog::ExistingFile);
	}
	else
	{
		myDlg->setWindowTitle(QObject::tr("导出"));
...
Handle(TopTools_HSequenceOfShape) Translate::importBREP(const QString& file)
{
	...
	TCollection_AsciiString  aFilePath = file.toStdU16String().data();
...
bool Translate::exportBREP(const QString& file, const Handle(TopTools_HSequenceOfShape)& shapes)
{
	...
	const TCollection_AsciiString anUtf8Path(file.toStdU16String().data());
