#ifndef _AIS_myManipulator_HeaderFile
#define _AIS_myManipulator_HeaderFile
...
NCOLLECTION_HSEQUENCE(AIS_myManipulatorObjectSequence, Handle(AIS_InteractiveObject))

DEFINE_STANDARD_HANDLE (AIS_myManipulator, AIS_InteractiveObject)
...
//! aManipulator->SetPart (0, AIS_myManipulator::Scaling, Standard_False);
//! aManipulator->SetPart (1, AIS_myManipulator::Rotation, Standard_False);
//! // Attach manipulator to already displayed object and manage manipulation modes
//! aManipulator->AttachToObject (anAISObject);
//! aManipulator->EnableMode (AIS_myManipulator::Translation);
//! aManipulator->EnableMode (AIS_myManipulator::Rotation);
//! aManipulator->EnableMode (AIS_myManipulator::Scaling);
//! @endcode
...
public:
  ...
  //! It is placed in the center of object bounding box, and its size is adjusted to the object bounding box.
  Standard_EXPORT void Attach (const Handle(AIS_myManipulatorObjectSequence)& theObject, const OptionsForAttach& theOptions = OptionsForAttach());
...
public:
  ...
  Standard_EXPORT Handle(AIS_myManipulatorObjectSequence) Objects() const;
  ...
  gp_Trsf StartTransformation (Standard_Integer theIndex) const
  {
    Standard_ProgramError_Raise_if (theIndex < 1 || theIndex > Objects()->Upper(),
      "AIS_myManipulator::StartTransformation(): theIndex is out of bounds");
...
public:
  ...
  DEFINE_STANDARD_RTTIEXT(AIS_myManipulator, AIS_InteractiveObject)
};
