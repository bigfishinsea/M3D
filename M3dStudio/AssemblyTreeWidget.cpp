...
AssemblyTreeWidget::AssemblyTreeWidget(QWidget* parent) : Parent(parent)
...
void AssemblyTreeWidget::UpdateAssemblyTree(Component* pRoot)
{
	m_root = pRoot;
...
AssemblyTreeWidget::~AssemblyTreeWidget()
{
	delete myImageShowDlg;
}

void AssemblyTreeWidget::init()
{
	count = 0;
	this->setIconSize(QSize(50, 31));
	this->setFrameStyle(QFrame::Panel | QFrame::Plain);
	this->setContextMenuPolicy(Qt::CustomContextMenu);
	//connect(this, &AssemblyTreeWidget::customContextMenuRequested, this, &AssemblyTreeWidget::show_menu);
...
//根据组件的名字，亮显item
void AssemblyTreeWidget::HighLightItem(const char* csItem)
{
	//遍历全部节点
...
//节点高亮
void AssemblyTreeWidget::curItemChangedSlot(QTreeWidgetItem* cur, QTreeWidgetItem* pre)
{
	if (!cur) return;
	static int times = 0;
	if (times == 1) {
...
void AssemblyTreeWidget::onTimeout()
{
	if (m_Clicked == 1) {
		pickupComp();
	}
	...
	if (!m_bTimeIsUp) //
	{
		QPixmap pm = GetPixmap(m_curr_item);
		if (!pm.isNull())
		{
			QString sItemText = m_curr_item->text(0);
			myImageShowDlg->setWindowTitle(sItemText);
			myImageShowDlg->setGeometry(m_globalPos.x() + 50, m_globalPos.y(), 810, 510);
...
QPixmap AssemblyTreeWidget::GetPixmap(QTreeWidgetItem* theItem)
{
	QPixmap pm;
...
void AssemblyTreeWidget::leaveEvent(QEvent* event)
{
	myImageShowDlg->hide();
	m_timer->stop();
}
...
//鼠标移动就不显示 了
//开始计时
void AssemblyTreeWidget::mouseMoveEvent(QMouseEvent* event)
{
	if(myImageShowDlg)
		myImageShowDlg->hide();
	if (m_timer->isActive())
	...
	QModelIndex index = this->indexAt(event->pos());
	{
		if (index.isValid())
		{
			m_curr_item = this->itemAt(event->pos());
			m_timer->start(1000);
			m_bTimeIsUp = false;
...
void AssemblyTreeWidget::mousePressEvent(QMouseEvent* event)
{
	//current=currentItem();
	QModelIndex index = this->indexAt(event->pos());
...
void AssemblyTreeWidget::mouseReleaseEvent(QMouseEvent* event)
{
...
void AssemblyTreeWidget::pickupComp()
{
...
TopoDS_Shape AssemblyTreeWidget::getTopoDS(Component* pCom) {
...
QSize AssemblyTreeWidget::sizeHint() const
{
	return QSize(Parent->width() / 3, Parent->height() / 2);
}
void AssemblyTreeWidget::initTree()
{
	this->clear();
	QString dataStr;
	QIcon icon;
	icon.addFile("images/Assemblylibrary.png");
	/*QTreeWidgetItem* item = new QTreeWidgetItem(AssemblyTreeWidget::itTopItem);
...
void AssemblyTreeWidget::dfs(QTreeWidgetItem* parItem,Component* myComp)
{
	if (!myComp) return;
...
/*void AssemblyTreeWidget::showTable(Component* myComp)
{
...
QTreeWidgetItem* AssemblyTreeWidget::addItem(QTreeWidgetItem* parItem,Component* myComp)
{
	int type=1002 ;
...
void AssemblyTreeWidget::init_menu() {
	menu = new QMenu(Parent);
	QAction* renameAct = new QAction(tr("重命名"), this);
	QAction* delAct = new QAction(tr("删  除"), this);
	QAction* repostureAct = new QAction(tr("调  姿"), this);
	//为添加关联函数
	connect(renameAct, &QAction::triggered, this, &AssemblyTreeWidget::renameComponent);
	connect(delAct, &QAction::triggered, this, &AssemblyTreeWidget::deleteComponent);
	connect(repostureAct, &QAction::triggered, this, &AssemblyTreeWidget::repostureComponent);
	menu->addAction(renameAct);
	menu->addAction(delAct);
	menu->addAction(repostureAct);
}


void AssemblyTreeWidget::show_menu(QMouseEvent* event)
{
...
void AssemblyTreeWidget::highlight(QTreeWidgetItem* parItem)
{
...
void AssemblyTreeWidget::repostureComponent()
{
	QTreeWidgetItem* myItem = current;
	...
	if (!compName.isEmpty())
	{
		...
		GeomWidget* pGeomW = (GeomWidget*)((MainWindow*)Parent)->getGeomWidget();
		View* view = pGeomW->GetView();
		view->NewRepostureCommand(); 
	}
}

void AssemblyTreeWidget::deleteComponent()
{
	//QTreeWidgetItem* parItem = this->currentItem();
	del(current);
}
...
//改名
//界面、内存、文档
void AssemblyTreeWidget::renameComponent()
{
	if (!current ){
		return;
	}
...
void AssemblyTreeWidget::del(QTreeWidgetItem* myItem)
{
	if (!myItem) {//|| myItem == this->topLevelItem(0)) 
		return;
	}
